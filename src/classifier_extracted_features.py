...
if __name__ == '__main__':
    ...
    logging.basicConfig(filename=os.path.join('logs', f'model_training_features_{pd.Timestamp.now().strftime("%Y%m%d_%H%M%S")}.log'), level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
    
    logging.info(f"Using feature extracted data")

    # Training set
    set_a = pd.read_parquet(os.path.join('loaded_data', 'a_patient_data_processed_cluster.parquet'))
    training_X =  pd.read_parquet(os.path.join('extracted_features', 'training_X_clean_custom_2.parquet')) #
    ...
    validation_X = pd.read_parquet(os.path.join('extracted_features', 'validation_X_clean_custom_2.parquet'))[training_X.columns]  #
    ...
    test_X = pd.read_parquet(os.path.join('extracted_features', 'test_X_clean_custom_2.parquet'))[training_X.columns] #  #
